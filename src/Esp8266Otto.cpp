...
{
  attachServos(); // Attach servos before moving

  int timeSpan = 10; // 10ms
...
void EspOtto::enableServoLimit(int diff_limit)
{
  for (int i = 0; i < 4; i++)
  {
    _servoDrivers[i].SetLimiter(diff_limit);
  }
}

void EspOtto::disableServoLimit()
{
  for (int i = 0; i < 4; i++)
  {
    _servoDrivers[i].DisableLimiter();
  }
}
